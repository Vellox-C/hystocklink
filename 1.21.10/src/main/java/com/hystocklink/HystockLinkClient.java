...
public class HystockLinkClient implements ClientModInitializer {
    ...
    private static final ResourceLocation PHASE_LAST =
            ResourceLocation.fromNamespaceAndPath("hystocklink", "last");
    ...
    public void onInitializeClient() {
        KeyMapping.Category category = KeyMapping.Category.register(
                ResourceLocation.fromNamespaceAndPath("hystocklink", "hystocklink")
        );
        KEY_MOVE = KeyBindingHelper.registerKeyBinding(new KeyMapping(
        ...
        ScreenEvents.AFTER_INIT.register((mc, screen, width, height) -> {
            ...
            // Render on top — afterRender fires after Screen.render() and all other mods' injections.
            ScreenEvents.afterRender(screen).register((s, graphics, mouseX, mouseY, delta) -> {
                if (activeWidget != null) {
                    activeWidget.render(graphics, mouseX, mouseY, delta);
                }
            });
