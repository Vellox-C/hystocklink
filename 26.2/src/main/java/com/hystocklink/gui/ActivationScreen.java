...
public class ActivationScreen extends Screen {
    ...
    private void activate() {
        ...
        Thread.ofVirtual().start(() -> {
            try {
                ...
                Thread.sleep(2000);
                Minecraft.getInstance().execute(() -> this.minecraft.setScreenAndShow(null));
            } catch (Exception e) {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        context.centeredText(this.font, this.title, width / 2, height / 2 - 40, 0xFFFFFFFF);
        context.centeredText(this.font,
                Component.literal("Paste your Minecraft access token (ygg.token)"),
                width / 2, height / 2 - 25, 0xFFAAAAAA);
        if (!statusMessage.isEmpty()) {
            context.centeredText(this.font, Component.literal(statusMessage), width / 2, height / 2 + 42, 0xFFFFFFFF);
        }
