...
public class AccountWidget extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.text(font, label, cx(), cy(), col, false);
        ...
        ctx.centeredText(font, "Activate Account", cx() + CONTENT_W / 2, btnY() + 6, btnTxt);

        // Hint text
        String keyName = HystockLinkClient.KEY_MOVE != null
                ? HystockLinkClient.KEY_MOVE.getTranslatedKeyMessage().getString()
                : "LCTRL";
        ctx.text(font, "Hold " + keyName + " to move", cx(), hintY(), col, false);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean b) {
        ...
        if (isMoveKeyHeld()) {
            ...
            Minecraft.getInstance().setScreenAndShow(new ActivationScreen());
        }
